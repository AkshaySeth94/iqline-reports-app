...
### ADR-styling: Tailwind CSS for UI Styling
**Decision:** The frontend will use Tailwind CSS for all styling to implement the dark theme and ensure a consistent, utility-first approach.
**Rationale:** The new requirement for a dark theme (FR-14) necessitates a systematic approach to styling. Tailwind CSS provides a comprehensive set of utility classes that make it easy to implement a dark theme using its `dark:` variant. It avoids the pitfalls of inline styles or inconsistent custom CSS files. It is the project's default styling system and is well-supported by the Next.js scaffolder. We trade away the component-based styling of libraries like Material UI, which is overkill for this project's needs.
...
7.  **Theming**: Apply dark theme (FR-14) across all frontend components using the new styling system.
