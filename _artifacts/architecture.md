...
1.  **Patient Application**: A mobile-first web interface (FR-1 to FR-5, FR-12, FR-14) for patients to log in via OTP, view their lab reports, and see a trend chart of their glucose levels.
2.  **Admin Panel**: A secure web interface (FR-6 to FR-11, FR-13, FR-14) for administrators to manage patient records and their associated lab reports.
...
-   **Web Frontend**: **Next.js 14.2**. It will serve both the Patient Application and the Admin Panel. Styling will be managed by Tailwind CSS to implement the required dark theme (FR-14) and ensure a consistent, mobile-first UI.
...
### ADR-styling-tailwind: Tailwind CSS for Styling
**Decision:** The frontend application will use the Tailwind CSS utility-first framework for all styling, including the implementation of the required dark theme.
**Rationale:** The introduction of a dark theme requirement (FR-14) necessitates a systematic approach to styling to ensure consistency and maintainability. Tailwind CSS provides a robust system for theming (including dark mode support via `dark:` variants), a comprehensive set of utility classes that speeds up development, and is automatically configured by the Next.js scaffolder. This avoids ad-hoc inline styles or CSS files, which are difficult to manage for a consistent theme. We trade away the component-based styling of libraries like Material UI, which is not required by the spec and would add more dependencies.
**Affects:** Frontend (all components), FR-14.
...
5.  **Frontend Foundation**: Set up the base dark theme and layout using Tailwind CSS (FR-14).
...
-   **FR-14 (Dark Theme)**: Covered by the decision to adopt Tailwind CSS (ADR-styling-tailwind).
